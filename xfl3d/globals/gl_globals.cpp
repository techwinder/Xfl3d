...
void gl::makeLines(QVector<Vector3d> const &points, QOpenGLBuffer &vbo)
{
 ...
 */
void gl::makeTriangles3Vtx(QVector<Triangle3d> const &triangles, bool bFlatNormals, QOpenGLBuffer &vbo)
{
    //Make surface triangulation
    int bufferSize = triangles.count();
...
void gl::makeTrianglesOutline(QVector<Triangle3d> const &triangles, const Vector3d &position, QOpenGLBuffer &vbo)
{
    int nPanel3 = triangles.size();
    // vertices array size:
    //        n triangular Panels
...
void gl::makeTriangleNormals(QVector<Triangle3d> const &trianglelist, float coef, QOpenGLBuffer &vbo)
{
    ...
    //      x2 nodes per normal
    //        x6 = 3 components
    int nodeVertexSize = trianglelist.count() * 2 * 3;
    QVector<float>nodeVertexArray(nodeVertexSize, 0);

    int iv=0;

    for (int p=0; p<trianglelist.count(); p++)
    {
...
void gl::makeTriangleNodeNormals(QVector<Triangle3d> const &trianglelist, float coef, QOpenGLBuffer &vbo)
{
    if(fabsf(coef)<1.0e-3f) coef = 1.0f;
    ...
    //      x2 nodes per normal
    //        x3 = 3 components
    int nodeVertexSize = trianglelist.count() * 3 * 2 * 3;
    QVector<float>nodeVertexArray(nodeVertexSize, 0);
...
void gl::makeNodeNormals(QVector<Node> const &nodelist, Vector3d const &pos, float coef, QOpenGLBuffer &vbo)
{
    // vertices array size:
    //        n  nodes
    //      x2 nodes per normal
    ...
    int nodeVertexSize = nodelist.count()* 2 * 3;
    QVector<float>nodeVertexArray(nodeVertexSize, 0);
...
void gl::makeNodeForces(QVector<Node> const &nodes, QVector<double> const &CpNodes,
                      float qDyn,
                      double &rmin, double &rmax, bool bAuto, double scale,
                      QOpenGLBuffer &vbo)
{
    int nNodes = nodes.size();
    if(nNodes==0 || nNodes>CpNodes.size())
    {
        vbo.destroy();
        return;
    }
...
//for(int in=0; in<nNodes; in++) qDebug(" %3d   %13g", nodes.at(in).index(), CpNodes.at(in));
    ...
    for (int p=0; p<nNodes; p++)
    {
        ...
        if(node.index()>=CpNodes.size()) break;
...
void gl::makeCpSection(QVector<Node> const&pts, QVector<double> const&Cp, double coef, QVector<Node> &cpsections)
{
