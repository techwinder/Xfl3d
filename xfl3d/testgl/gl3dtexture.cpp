...
void gl3dTexture::onSaveImg()
{
    ...
    if(!loc.isEmpty()) s_LastFileName = loc.first()+QDir::separator();
...
void gl3dTexture::glMakeTexSphere(int nSplits)
{
    ...
    int bufferSize = icotriangles.count();
    ...
    float U(0), V(0);
    int iv = 0;
    for(int it=0; it<icotriangles.size(); it++)
    {
...
void gl3dTexture::glMake3dObjects()
{
    ...
    {
        if(m_pTexture) delete m_pTexture;
        m_pTexture = new QOpenGLTexture(*m_pImg);
