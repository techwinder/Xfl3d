...
/*    Node A(m_GLViewRect.left(),                              -m_GLViewRect.top(),                               -1.0);
    Node B(m_GLViewRect.left() + 2.0 * m_GLViewRect.width(), -m_GLViewRect.top(),                               -1.0);
    Node C(m_GLViewRect.left() + 2.0 * m_GLViewRect.width(),  m_GLViewRect.top() - 2.0 * m_GLViewRect.height(), -1.0);
    Node D(m_GLViewRect.left(),                               m_GLViewRect.top() - 2.0 * m_GLViewRect.height(), -1.0);*/
    Node A(-1.0, -1.0, -1.0);
    Node B(-1.0,  1.0, -1.0);
    Node C( 1.0,  1.0, -1.0);
    Node D( 1.0, -1.0, -1.0);
    ...
    gl::makeQuad(A, B, C, D, m_vboBackgroundQuad);
...
/**
 * @brief gl3dView::setScale defines the reference length, and updates the view.
 */
...
void gl3dView::startResetTimer()
{
    ...
    m_glScaleIncrement = (referenceScale()-m_glScalef)/m_nAnimationFrames;
    m_TransIncrement = (Vector3d(0.0,0.0,0.0)-m_glViewportTrans)/m_nAnimationFrames;
...
void gl3dView::onResetIncrement()
{
    ...
    double frac = double(m_iTimerInc)/double(m_nAnimationFrames) * 3.14592654/2.0;
    double tau = sin(frac);
    m_glScalef += (referenceScale()-m_glScalef)*tau;
    m_glViewportTrans += (Vector3d(0.0,0.0,0.0)-m_glViewportTrans)*tau;
...
/** resets the new rotation center after a translation or a rotation */
...
{
    m_ImagePath = QFileDialog::getOpenFileName(this, tr("Open Image File"),
...
void gl3dView::drawBackImage(QPainter &painter)
{
