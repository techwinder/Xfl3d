...
QString Quaternion::listQuaternion() const
{
    QString strange = QString::asprintf("a=%g  q=(%g, %g, %g)\n", a, qx, qy, qz);
    strange += QString::asprintf("Axis  = %7.3f, %7.3f, %7.3f\n", axis().x, axis().y, axis().z);
    strange += QString::asprintf("Angle = %.3f", angle()) + DEGCHAR + EOLCHAR;
    strange += QString::asprintf("Norm = %g", norm()) + EOLCHAR;
    return strange;
}
