...
{
    pslg.append({m_S[0],m_S[1]});
    pslg.append({m_S[1],m_S[2]});
    pslg.append({m_S[2],m_S[0]});
}
...
QString Triangle2d::properties(bool bLong) const
{
    QString props;
    QString strange;
    ...
    for(int in=0; in<3; in++)
    {
        strange = QString::asprintf("  Node(%d) = (%7.2f, %7.2f) ", in,
                        m_S[in].x*Units::mtoUnit(), m_S[in].y*Units::mtoUnit());
        strange += Units::lengthUnitLabel() + "\n";
        props += strange;
    }

    strange = QString::asprintf("  CoG     = (%7.2f, %7.2f) ", CoG_G.x*Units::mtoUnit(), CoG_G.y*Units::mtoUnit());
    strange += Units::lengthUnitLabel() + "\n";
    props += strange;

    strange = QString::asprintf("  Area    = %11g ", m_SignedArea*Units::m2toUnit());
    props += strange + Units::areaUnitLabel() + "\n";

    double r=0,e=0;
    double qual = qualityFactor(r,e);

    strange = QString::asprintf("  Max. edge length = %9g ", maxEdgeLength()*Units::mtoUnit());
    strange += Units::lengthUnitLabel() + "\n";
    props += strange;

    strange = QString::asprintf("  Min. edge length = %9g ", e*Units::mtoUnit());
    strange += Units::lengthUnitLabel() + "\n";
    props += strange;

    strange = QString::asprintf("  Angles = (%5.1f, %5.1f, %5.1f) ", m_Angle[0], m_Angle[1], m_Angle[2]);
    props += strange + DEGCHAR + "\n";


    strange = QString::asprintf("  Quality factor   = %5f", qual);
    if(qual>s_QualityBound) strange += " - skinny";
    props += strange + "\n";
